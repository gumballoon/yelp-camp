...
const dbURL = process.env.DB_URL; // production DB vs. development DB
mongoose
  .connect(dbURL) // to connect to a specific database
  .then(() => {
...
app.use((err, req, res, next) => {
  ...
  res.status(err.status || 404).render("campgrounds/error", { err });
});
